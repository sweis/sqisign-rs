...
const P_HI: u64 = 0x04FF_FFFF_FFFF_FFFF; // p[3]; p[0..2] = u64::MAX
const TWO_P_HI: u64 = 0x09FF_FFFF_FFFF_FFFF; // (2p)[3]; (2p)[0] = -2, (2p)[1,2] = -1

/// Butterfly: `(a+b, a−b)` with both outputs fully reduced (< 2²⁵¹).
/// 4 ADD/ADC, 4 SUB/SBB, 2× cond-sub-p on the sum, 1× cond-add-2p +
/// 1× cond-sub-p on the diff.
#[inline(always)]
pub fn fp_addsub(sum: &mut [u64; 4], diff: &mut [u64; 4], a: &[u64; 4], b: &[u64; 4]) {
    let (s0, s1, s2, s3, d0, d1, d2, d3): (u64, u64, u64, u64, u64, u64, u64, u64);
    unsafe {
        asm!(
            // Load a, copy for diff.
            "mov  r8,  [{a}]",
            "mov  r9,  [{a}+8]",
            "mov  r10, [{a}+16]",
            "mov  r11, [{a}+24]",
            ...
            "adc  r10, [{b}+16]",
            "adc  r11, [{b}+24]",
            // diff = a − b  (r12..r15), capture borrow as −1/0.
            "sub  r12, [{b}]",
            "sbb  r13, [{b}+8]",
            "sbb  r14, [{b}+16]",
            "sbb  r15, [{b}+24]",
            "sbb  rax, rax",
            // ---- reduce sum: 2× cond-sub-p (p[0..2]=−1, p[3]=P_HI) ----
            "mov  rdx, r11",
            "shr  rdx, 59",
            "neg  rdx",
            "mov  rcx, {p3}",
            ...
            "sbb  r10, rdx",
            "sbb  r11, rcx",
            // ---- reduce diff: cond-add-2p on borrow, then cond-sub-p ----
            // 2p[0]=−2, 2p[1,2]=−1, 2p[3]=TWO_P_HI; mask all by rax.
            "mov  rdx, -2",
            "mov  rcx, {p2_3}",
            "and  rdx, rax",
            "and  rcx, rax",
            ...
            out("r8") s0, out("r9") s1, out("r10") s2, out("r11") s3,
            out("r12") d0, out("r13") d1, out("r14") d2, out("r15") d3,
...
/// Lazy butterfly: `(a+b, a−b+2p·𝟙[a<b])` with both outputs < 2²⁵²
/// (one reduction round each). Safe as `fp_mul`/`fp_sqr` operands;
/// **not** as `fp_sumprod` operands (that allows only one < 2²⁵²).
#[inline(always)]
pub fn fp_addsub_lazy(sum: &mut [u64; 4], diff: &mut [u64; 4], a: &[u64; 4], b: &[u64; 4]) {
    let (s0, s1, s2, s3, d0, d1, d2, d3): (u64, u64, u64, u64, u64, u64, u64, u64);
    unsafe {
        asm!(
            "mov  r8,  [{a}]",
            ...
            "sbb  r10, rdx",
            "sbb  r11, rcx",
            // diff: cond-add-2p on borrow → < 2p < 2²⁵²
            "mov  rdx, -2",
            "mov  rcx, {p2_3}",
            "and  rdx, rax",
            "and  rcx, rax",
