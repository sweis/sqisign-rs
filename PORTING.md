...
- **Supersingularity check (spec steps 3-4)** is implicit in C (per spec §4.5 prose: "byproduct of a successful computation of an isogeny"). C checks `ec_curve_verify_A` (A∉{±2}; A=0 is *accepted* — it is the supersingular E₀) for both curves, then relies on isogeny computation to fail for non-supersingular input. Matches spec's stated implementation strategy.
...
- **DoS via adversarial `find_nqr_factor` input** (basis.c): a public-key or `sig.e_aux_a` curve with Re(A²)=2 (e.g. A=√2 ∈ Fp, valid since p≡7 mod 8) and `hint=1` makes the entangled-basis NQR search loop forever — Im(ib·A²−(1+ib)²) vanishes for every b, so t0 ∈ Fp and `fp2_is_square(t0)` is always true. C's `do { } while(!found)` has no cap. Rust now caps the search at 256 candidates and rejects. Regression test `reject_adversarial_nqr_dos`.
...
- **Non-canonical Fp2 encodings accepted** (encode_verification.c): `fp2_decode` returns 0 on input ≥ p but the decode wrappers discard the status, so any garbage high-byte field maps to A=0 (j=1728). Second malleability vector. Rust now propagates the status and rejects in `sqisign_verify`. Regression test `reject_noncanonical_encoding`.
- **`fp2_select` called with ctl=1** (ec_jac.c:145,192): C's `-flag` on a `uint32_t` mask yields 1, violating the 0/-1 contract; harmless because the doubling formula already gives O for O. Rust passes the mask directly.
...
| Loop bounds | `find_na_x_coord`/`find_nqr_factor` loops are now hard-capped at 256; an adversarial A can otherwise make `find_nqr_factor` non-terminating (see upstream findings). All other verify-path loops bounded by `TORSION_EVEN_POWER`/`SQISIGN_RESPONSE_LENGTH` constants. | **Fixed** |
| `lift_basis` with P=O | An all-zero basis-change matrix yields P=O; the batched inverse then silently zeroes `e.a`. | **Fixed**: bail if `b.p.z` or `e.c` is zero. Test `reject_zero_matrix_gracefully`. |
| `multiple_mp_shiftl(..,0,..)` | Funnel shift by `RADIX-0` is UB in C, panic in Rust debug. No live caller passes 0 but no defensive guard either. | **Fixed**: shift==0 is a no-op. Test `multiple_shiftl_zero_is_nop`. |
...
| Secret zeroization | `DrbgState` derives `ZeroizeOnDrop`; `aes` is built with its `zeroize` feature so the round-key schedule is scrubbed. `SecretKey::Drop` zeroes field-element/basis components and calls `ibz_secure_clear` on every bigint field (rug: zeroes the GMP limb buffer via `mpz_t`; crypto-bigint: zeroes the stack `[u64; L]`; malachite: best-effort bit-by-bit clear — no public mutable limb access). `ibz_to_bytes`/`from_bytes` use `Zeroizing<Vec>` for the temporary limb buffer. **Limitations**: bigint backends may realloc; freed intermediate buffers from prior allocations are not covered. Signing-path locals (`lideal_chall_secret`, `resp_quat`, etc.) are not yet scrubbed — full intermediate-zeroize requires backend-agnostic `Zeroize` on `QuatLattice`/`QuatLeftIdeal`. The serialized sk byte buffer should also be zeroized by callers. | Best-effort |
